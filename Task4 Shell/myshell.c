    ...
    while (*cmd_array != NULL) {
        ...
        switch (res) {
            ...
            case BG:
                cmd_array++;
                cmd_struct->bg_fl = 1;
                if (cmd_struct->command_array == NULL) {
                    printf("There is nothing before &\n");
                    return root;
                }
                else {
...
int executing_a_command(command *cmd_struct) {
    ...
    int fd[2], file, status = 0, cnt_flg = 1;  // continue_flag
    while (cmd_struct != NULL) {
        if (cmd_struct->and_fl) {
            cnt_flg = !status;
        }
        if (cmd_struct->or_fl) {
            cnt_flg = status;
        ...
        if ((cmd_struct->command_array != NULL) && (!(strcmp(cmd_struct->command_array[0], "cd")))) {
            ...
            if (!pid) {  // if True - child, else - father
                if (cmd_struct->bg_fl) {
                    printf("[%d] is started\n", getpid());
                    int bg_file;
                    bg_file = open("/dev/null", O_RDONLY);
                    if (bg_file == -1) {
                ...
                if (!cmd_struct->bg_fl) { // Finish the process with Ctrl+C if it's not BG
...
void kill_them() {
    ...
    while ((pid = waitpid(-1, &status, WNOHANG)) > 0) {
        if (WIFEXITED(status)) {
            printf("[%d] is done with status %d\n", pid, status);
        } else {
            printf("[%d] is done with error\n", pid);
        }
    }
}
